...
const [isRedeeming, setIsRedeeming] = useState(false);
  ...
  const balance      = vendorData?.wallet_balance ?? 0;
  const fname        = vendor.fname || localStorage.getItem("fname") || "User";
  ...
  const referred = vendorData?.refered_to || [];

  const handleCopy = () => {
    navigator.clipboard.writeText(vendor.my_id || "").catch(() => {});
    setCopied(true);
...
const handleRedeem = async () => {
  ...
  try {
    setIsRedeeming(true);
    const res = await Apiconnect.postData("vendor/wallet_redeem", { amount: selectedAmt });
    if (res.data?.status === 1 || res.data?.status === "1") {
       toast.success(
        isHindi
          ? "राशि सफलतापूर्वक रिडीम हुई!"
     ...
     toast.error(
        res.data?.message ||
          (isHindi ? "रिडीम विफल हुआ" : "Redeem failed")
      );
    }
  } catch (err) {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", color: "#fff", fontFamily: "sans-serif" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}> {isHindi ? "वॉलेट लोड हो रहा है..." : "Loading wallet…"}</div>
        </div>
  ...
  return (
    <>
      <style>{`
        ...
        .wal-txn-id    { font-size: 10px; color: rgba(255,255,255,0.25); font-family: monospace; margin-top: 1px; }
        ...
        <div className="wal-card">
          <div className="wal-card-inner">
            ...
            <div className="wal-card-head">
              ...
              <div>
                <div className="wal-card-label">{isHindi ? "सनातन वॉलेट" : "Sanatan Wallet"}</div>
                <div className="wal-card-user">{fname}</div>
              </div>
            </div>

            <div className="wal-bal-label">{isHindi ? "उपलब्ध बैलेंस" : "Available Balance"}</div>
            <div className="wal-bal-amount">
            ...
            <div className="wal-stats">
              <div className="wal-stat">
                ...
                <div className="wal-stat-val">₹{totalEarned.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</div>
                <div className="wal-stat-lbl">{isHindi ? "कुल कमाई" : "Total Earned"}</div>
              </div>
              <div className="wal-stat">
                <span className="material-symbols-outlined msym" style={{ fontVariationSettings: "'FILL' 1" }}>redeem</span>
                <div className="wal-stat-val">₹{referBonus.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</div>
                <div className="wal-stat-lbl">{isHindi ? "रेफरल बोनस" : "Referral Bonus"}</div>
              </div>
              <div className="wal-stat">
                <span className="material-symbols-outlined msym" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                <div className="wal-stat-val">₹{cashback.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</div>
                <div className="wal-stat-lbl">{isHindi ? "कैशबैक" : "Cashback"}</div>
              </div>
            </div>

            {/* Replace the single wal-add-btn with this */}
        ...
        <div className="wal-section">
          ...
          {transactions.length === 0 ? (
            <div className="wal-empty">
             ...
             {isHindi ? "अभी तक कोई लेनदेन नहीं" : "No transactions yet"}
            </div>
          ) : (
            transactions.map((t) => {
                ...
                <div className="wal-txn" key={t.id}>
                  ...
                  <div className="wal-txn-info">
                    ...
                    <div className="wal-txn-date">
                      {fmtDate(t.created_on)}{t.payment_mode ? ` · ${t.payment_mode}` : ""}
                    </div>
...
>
  {isRedeeming ? (
    <>
      ...
      {isHindi ? "रिडीम हो रहा है..." : "Redeeming…"}
    </>
  ) : (
