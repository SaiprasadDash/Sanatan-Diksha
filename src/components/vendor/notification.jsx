...
const fmtDate = (str, isHindi = false) => {
  if (!str) return "";
  ...
  try {
    ...
    if (diff < 3600)
      return `${Math.floor(diff / 60)}${isHindi ? "मि पहले" : "m ago"}`;

    if (diff < 86400)
      return `${Math.floor(diff / 3600)}${isHindi ? "घं पहले" : "h ago"}`;

    return `${Math.floor(diff / 86400)}${isHindi ? "दिन पहले" : "d ago"}`;
  } catch {
    return str;
  }
};
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const res = await Apiconnect.postData("vendor/notifications");
    ...
    if (isOpening && n.msg_seen !== 1) {
      try {
        const res = await Apiconnect.postData(
          `vendor/notifications/read/${n.id}`
        );
        ...
        {loading ? (
          ...
          notifications.map((n) => {
            ...
            return (
              <div
                key={n.id}
                className={`notif-card ${isOpen ? "open" : ""} ${
                  isUnread ? "unread" : ""
                }`}
              >
                <div
                ...
                >
                  ...
                  {/* <span className="notif-time">
                    {fmtDate(n.created_on, isHindi)}
                  </span> */}

                  <i
                    className={`material-symbols-outlined notif-chevron ${
                      isOpen ? "open" : ""
                    }`}
                  >
                    expand_more
