...
const Reviews = () => {
  ...
  useEffect(() => {
    const loadReviews = async () => {
      setLoading(true);
      try {
        const res = await Apiconnect.postData("vendor/reviews_list");
  ...
  const filtered = filter === 0 ? reviews : reviews.filter(r => r.star_rating === filter);
  ...
  const fiveStarPct = reviews.length
    ? Math.round((reviews.filter(r => r.star_rating === 5).length / reviews.length) * 100)
    : 0;
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; transition: all 0.25s ease-in-out; }
        ...
        .rv-filter-btn:hover { background: rgba(255,255,255,0.1); color: #fff; }
        ...
        .rv-card-left { display: flex; align-items: center; gap: 8px; }
        .rv-reviewer {
          font-size: 14px;
          font-weight: 500;
          color: #fff;
      ...
      <div className="rv-page">
        ...
        <div className="rv-filters">
          <button
            className={`rv-filter-btn ${filter === 0 ? "rv-active" : ""}`}
            onClick={() => setFilter(0)}
          >
            {isHindi ? "सभी" : "All"}
          </button>
          ...
          {[5, 4, 3, 2, 1].map(n => (
            <button
              key={n}
              className={`rv-filter-btn ${filter === n ? "rv-active" : ""}`}
              onClick={() => setFilter(n)}
            >
        ...
        <div className="rv-list">
          {loading ? (
            <div className="rv-loading">
              ...
              {isHindi ? "समीक्षाएँ लोड हो रही हैं..." : "Loading reviews..."}
            </div>
          ) : filtered.length === 0 ? (
            <div className="rv-empty">
              {isHindi
                ? "इस रेटिंग के लिए कोई समीक्षा नहीं है।"
              ...
              <div key={r.id} className="rv-card">
                <div className="rv-card-header">
                  <div className="rv-card-left">
                    <span className="rv-reviewer">{r.customer_name}</span>
                ...
                {/* {r.service_id && (
                  <p className="rv-service">
                    {isHindi ? "सेवा" : "Service"} #{r.service_id}
                  </p>
                )} */}
              </div>
            ))
